...
def render_tools_section() -> List[str]:
    ...
    tools = st.multiselect(
        ...
        [
            ...
            *[agent["name"] for agent in BUILDERS_TOOLBOX],
        ],
...
def render_new_tools_section() -> List[str]:
    st.markdown("#### 🔶 Section 5 — New AI Tools Created")
    raw = st.text_area(
        "List any new AI tools, agents, or components you're building specifically for this solution (comma or newline separated).",
        height=100,
        key="solution_new_ai_tools",
    )
    return [item.strip() for item in re.split(r"[\n,]+", raw) if item.strip()]


def render_so_what_section() -> dict:
    st.markdown("#### 🔶 Section 6 — The SO WHAT (Business Benefits & Impact)")
...
def render_components_section() -> List[dict]:
    st.markdown("#### 🔶 Section 7 — The HOW (Architecture / Components)")
...
def render_skills_comparison(profile_skills: List[str]) -> tuple[List[dict], List[str]]:
    st.markdown("#### 🔶 Section 8 — Skills Needed VS My Skills VS Help Needed")
...
def build_solution_entry(
    ...
    return {
        ...
        "ai_tools_used": tools,
        "new_ai_tools_created": new_ai_tools,
        "so_what": so_what,
...
def render_solution_form(submissions: List[dict], solutions: List[dict]) -> None:
    ...
    new_ai_tools = render_new_tools_section()
    ...
    builders_toolbox = [
        {"Agent / Tool": agent["name"], "Reusable For": agent["description"]}
        for agent in BUILDERS_TOOLBOX
    ]
    st.subheader("🏗️ BUILDERS TOOLBOX : Generic AI Agents and Tools that can be reused for New Solutions")
    st.table(builders_toolbox)
            ...
            [
                {
                    ...
                    "AI tools Reused": ", ".join(idea.get("ai_tools_used", [])) or "—",
                    "New AI Tools created": ", ".join(idea.get("new_ai_tools_created", [])) or "—",
